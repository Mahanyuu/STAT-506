...
```
The result shows that torque has significantly negative effect on MPG on the highway at 1% level. I also choose to log the torque because torque is large and continuous data. And if the predictor of interest and dependent variable are logged, it is more clear to interpret.

For estimator, with the control of three dimensions, released year and the horsepower, when the torque increases by 100%, the MPG on the highway will significantly decrease by 66.5% at 1% level.
