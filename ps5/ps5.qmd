...
for (i in seq_along(genres)){
  ...
  p_yg <- p_yg %>%
    add_trace(data=artmedian[artmedian$genre == genres[i],], 
          x=~year, y=~price_usd, type='scatter', mode='line', name=genres[i],
          showlegend=TRUE, line=list(color=genre_color))
  p_yg <- p_yg %>%
    add_trace(data=art975[art975$genre==genres[i],],
          x=~year, y=~price_usd, type='scatter', mode='line',
          name=genres[i], showlegend=FALSE, line=list(dash='dash', color=genre_color))
}
p_yg
```
